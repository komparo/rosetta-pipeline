...
Briefly, a pipeline framework specifies a set of jobs (**specification**), each with some **inputs** and **outputs**, and executes theses jobs (**execution**). It also provides a user interface to develop, run and explore pipelines (**usability**), and can also keep track of which jobs were used to produce a particular output (**provenance**).
...
When jobs share some inputs and/or output, for example the same command, they are often grouped into a set of _rules_.

Rules are also known as processes (nextflow), tools (CWL) and tasks (WDL).

There are several ways to create individual jobs based on the specification of a rule:
...
Dependencies between jobs can be directly specified between jobs (examples: luigi) or between output and input data (examples: snakemake, nextflow, cwl). In the end, this produces a directed acyclic graph (DAG), which is executed starting from those jobs without any ingoing edges, and ends with those jobs without any outgoing edges.
...
- Number of inputs and outputs not known from the start: snakemake, nextflow. Related jobs: `r link_to_jobs("Unknown number of inputs")`
- Alternative paths based on the output of previous jobs: nextflow. Related jobs: `r link_to_jobs("Alternative paths")`
...
- The module is integrated in the pipeline as-is. Examples: snakemake (include)
- The module is included as a job within the pipeline, with its own input and output. Examples: snakemake (subworkflows), WDL (subworkflows).

If the pipeline is specified in a multi-purpose programming language, modularity of the pipeline is also possible within the language itself. Examples: airflow
...
Related jobs: `r link_to_jobs("Environment")`
...
The reproducibility of a pipeline hinges on how much of the inputs are specified.
...
Related jobs: `r link_to_jobs("Caching")`
