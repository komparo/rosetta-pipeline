...
link_to_tasks <- function(concept) {
    tasks %>%
...
Also known as: _workflow managers_, _workflow frameworks_
...
Dependencies between tasks can be directly specified between tasks (examples: luigi) or between output and input data (examples: snakemake, nextflow, cwl). In the end, this produces a directed acyclic graph (DAG), which is executed starting from those tasks without any ingoing edges, and ends with those tasks without any outgoing edges.

#### Pull vs push

There are two main models to create a DAG from a specification: push and pull.

In the _push_ model (also known as _data flow_), a task is ran once inputs are available. The workflow starts from those tasks for which all inputs are available, and uses the output of these tasks to run other tasks which depend on these outputs. Examples: nextflow, WDL
...
- Number of inputs and outputs not known from the start: snakemake, nextflow. Related tasks: `r link_to_tasks("Unknown number of inputs")`
- Alternative paths based on the output of previous tasks: nextflow. Related tasks: `r link_to_tasks("Alternative paths")`
...
Related tasks: `r link_to_tasks("Environment")`
...
- Only make specified input files available to the task: Cromwell, nextflow
...
Related tasks: `r link_to_tasks("Caching")`
...
During development, it can be useful to enter the workflow with a shell at a particular point, with the complete environment and inputs available.

### Debugging

To debug a particular task, it can be useful to enter the workflow with a shell at a particular point, with the complete environment and inputs available.
