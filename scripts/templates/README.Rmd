...
- **[Example workflows for each framework with some common workflow tasks](tasks)**. They showcase how each framework is used, and where the similarities and differences lie. This is inspired by [Rosetta code](http://www.rosettacode.org/wiki/Rosetta_Code).
...
link <- function(label, url) {paste0("[", label, "](tasks/", url, ")")}
walk(tasks, function(task) {
  cat(link(task$name, paste0("tasks/", task$id)))
  cat(": ")
  map(task$framework_ids, ~link(., paste0("tasks/", task$id, "/", .))) %>%
    paste0(collapse = " ") %>%
    cat()
  cat("\\\n\\\n")
})
...
4. Run snakemake: `snakemake`. A first build can take a while because all docker containers have to be built.
